...
const drawBackground = (ctx: CanvasRenderingContext2D): void => {
  ...
  gradient.addColorStop(0.52, "#f4eeff");
...
export const generateWhereMyRobocosanBeganImage = async (
  ...
  ctx.fill();
  ctx.strokeStyle = "rgba(180, 130, 220, 0.56)";
  ctx.lineWidth = 2;
  ctx.stroke();

  const titleText =
    options.locale === "en"
      ? "Where did your Roboco-san journey begin?"
      : "あなたのロボ子さんはどこから？";
  ...
  ctx.fillStyle = "rgba(160, 100, 220, 0.26)";
  ctx.beginPath();
  ctx.arc(
    iconX + iconSize / 2,
    iconY + iconSize / 2,
  ...
  ctx.strokeStyle = "rgba(160, 90, 200, 0.9)";
  ctx.lineWidth = 6;
  ctx.lineCap = "round";
  ...
  if (favoriteOriginalSong) {
    ...
    ctx.fillStyle = "rgba(94, 30, 122, 0.95)";
    ...
    ctx.fill();
    ctx.strokeStyle = "rgba(160, 90, 200, 0.45)";
    ctx.lineWidth = 2;
    ctx.stroke();
    ...
    ctx.fillStyle = "rgba(210, 180, 240, 0.72)";
    ctx.fill();
