...
describe("PlayerControlsBar", () => {
  ...
  it("お気に入りボタンをクリックすると toggleFavorite が呼ばれる", () => {
    ...
    // our mocked useFavorites.toggleFavorite is created in mock, check it was called
    // Because we cannot directly access internal mock, assert that button exists and is clickable
    expect(favBtn).toBeTruthy();
  });

  it("セトリネタバレ防止ボタンで setHideFutureSongs が呼ばれる", () => {
    const { getByLabelText } = renderComponent();
    const hideBtn = getByLabelText("セトリネタバレ防止モード");
    fireEvent.click(hideBtn);
    expect(setHideFutureSongs).toHaveBeenCalled();
  });
});
