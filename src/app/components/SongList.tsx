...
const SongsList = ({ songs, currentSongInfo, changeCurrentSong }: SongListProps) => {
    return (
        <ul className="song-list grid grid-cols-3 auto-rows-max md:grid-cols-1 lg:grid-cols-2 xl:grid-cols-3 2xl:grid-cols-4 gap-2 overflow-y-auto h-dvh lg:h-full flex-grow dark:text-gray-300">
            {songs.map((song, index) => (
                <li
                    key={index}
                    className={`p-3 rounded relative cursor-pointer transition ${currentSongInfo?.title === song.title && currentSongInfo.video_id === song.video_id ? 'bg-primary-300 hover:bg-primary-400 dark:bg-primary-900 dark:hover:bg-primary-800 dark:text-gray-300' : 'bg-gray-200 dark:bg-gray-700 hover:bg-gray-300 dark:hover:bg-gray-600'}`}
